-- Adjust version branch DELETE to only remove versions at or after the new effective_from,
-- instead of all versions after today. This preserves intermediate versions when scheduling
-- a future version change (e.g., from November while today is September).
...
BEGIN
    ...
    IF v_kind = 'version' THEN
        ...
        -- Only remove versions at or after the new effective_from, preserving
        -- any intermediate versions between today and the new effective date.
