...
export async function getUserLocationsMap(supabase: Awaited<ReturnType<typeof requireUser>>["supabase"], userId: string): Promise<Map<string, LocationOption>> {
    const { data, error } = await supabase.from("user_locations").select("id, name").eq("user_id", userId);
