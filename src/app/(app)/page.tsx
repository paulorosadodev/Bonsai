...
export default async function ResumoPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
    ...
    const [dashboard, categories, generalTags, specificTags] = await Promise.all([
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            {monthEmpty ? <EmptyState title="Ainda não há gastos neste mês" description="Quando as transações existirem, o resumo aparece aqui." /> : <CategoryChart items={dashboard.byCategory} />}
            {hasHistory ? <HistoryChart items={dashboard.history} /> : null}
            <DashboardEntriesSection
                entries={dashboard.entries}
                totalCents={dashboard.totalCents}
                filteredTotalCents={dashboard.filteredTotalCents}
                filters={filters}
