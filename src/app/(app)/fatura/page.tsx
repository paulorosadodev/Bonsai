...
import { InvoiceCycleCard } from "@/components/features/invoice-cycle-card";
...
export default async function InvoicePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            <TransactionFilters values={{ ...filters, month: invoice.month }} categories={categories} generalTags={generalTags} specificTags={specificTags} showPaymentMethod={false} searchPlaceholder="Buscar na fatura..." />
            {hasActiveFilters && invoice.entries.length > 0 ? (
                <div className="flex items-center justify-between px-1 text-xs text-muted">
                    ...
                    <span className="font-medium text-text">Total filtrado: {formatBrl(invoice.filteredTotalCents)}</span>
                </div>
            ) : null}
            {invoice.entries.length === 0 ? <EmptyState title={hasActiveFilters ? "Nenhuma parcela neste filtro" : "Nenhuma parcela nesta fatura"} description={hasActiveFilters ? "Tente ajustar ou limpar os filtros para ver outras parcelas." : "Compras no cartão aparecem aqui, agrupadas pelo vencimento."} /> : <InvoiceEntries entries={invoice.entries} sort={filters.sort} />}
        </div>
    );
}
