...
export async function updateCategory(id: string, input: CategoryUpdateInput): Promise<ActionResult<{ id: string }>> {
    ...
    const { data, error } = await supabase.from("user_categories").update(updateData).eq("id", idParsed.data).eq("user_id", user.id).select("id").maybeSingle();
...
export async function deleteCategory(id: string): Promise<ActionResult> {
    ...
    const { count: txCount } = await supabase.from("transactions").select("id", { count: "exact", head: true }).eq("category_id", idParsed.data).eq("user_id", user.id);
    ...
    const { count: recCount } = await supabase.from("recurring_versions").select("series_id", { count: "exact", head: true }).eq("category_id", idParsed.data);

    if (recCount && recCount > 0) {
        return { ok: false, error: "Esta categoria está em uso por recorrências ativas e não pode ser excluída." };
    }

    const { data, error } = await supabase.from("user_categories").delete().eq("id", idParsed.data).eq("user_id", user.id).select("id").maybeSingle();
